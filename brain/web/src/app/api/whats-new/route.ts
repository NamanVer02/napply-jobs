...
import { userDataRoot, readApplications } from "@/lib/career-ops";
...
export async function GET(req: Request) {
  ...
  let rows: string[];
  try {
    rows = fs.readFileSync(path.join(userDataRoot(session), "data", "scan-history.tsv"), "utf8").split("\n");
  } catch {
    return Response.json({ offers: [], count: 0 });
  }

  // Companies already evaluated → don't resurface as "new".
  const evaluated = new Set(readApplications(session).map((a) => norm(a.company)).filter(Boolean));
