...
import { careerOpsRoot, userDataRoot, readMemory } from "@/lib/career-ops";
...
//
// MULTI-TENANT: the prompt tells the CLI to write user-layer files relative to
// the user's data directory (users/<username>/). System scripts stay at the repo
// root. The `userPrefix` is either "" (single-user) or "users/<name>/" — it's
// prepended to every user-layer path in the prompt so the CLI writes to the right
// place. CLI cwd stays careerOpsRoot() so system scripts resolve correctly.
function buildPrompt(kind: string, input: string, memory: string, today: string, userPrefix: string): string {
...
1. Read modes/pdf.md, ${userPrefix}cv.md, ${userPrefix}config/profile.yml, and the evaluation report at ${userPrefix}reports/${input}-*.md (for the JD keywords + analysis).
...
4. Render the PDF: \`node generate-pdf.mjs /tmp/cv-{candidate}-{company}.html ${userPrefix}output/cv-{candidate}-{company}-${today}.pdf --format={letter for US/Canada companies, else a4}\`.
5. Update the tracker: in ${userPrefix}data/applications.md, change the PDF column for row #${input} from ❌ to ✅.
...
1. Read modes/oferta.md and follow it EXACTLY (blocks A–F, G posting-legitimacy, and the Machine Summary). Ground the fit in THIS person: read ${userPrefix}cv.md, ${userPrefix}config/profile.yml and ${userPrefix}modes/_profile.md. Use WebFetch to read the posting (you are headless — Playwright is unavailable, so use WebFetch and mark the report header "Verification: unconfirmed (batch mode)").

2. Persist the result CANONICALLY so the web and the CLI share ONE source of truth:
   a. Reserve a report number: run \`node reserve-report-num.mjs\` — its stdout is a 3-digit number (e.g. 035).
   b. Write the full report to ${userPrefix}reports/{num}-{company-slug}-${today}.md  (company-slug = company lowercased, non-alphanumerics → hyphens).
   c. Append ONE row of 9 TAB-separated columns to batch/tracker-additions/{num}-{company-slug}.tsv, in THIS exact order (real \\t tabs, status BEFORE score):
      {num}\t${today}\t{Company}\t{Role}\t{CanonicalStatus e.g. Evaluated}\t{score}/5\t❌\t[{num}](${userPrefix}reports/{num}-{company-slug}-${today}.md)\t{one-line note}
   d. Merge into the tracker: run \`node merge-tracker.mjs${userPrefix ? ` --tracker ${userPrefix}data/applications.md` : ""}\` (it dedupes by company+role+report-num, validates the status, and writes ${userPrefix}data/applications.md — NEVER edit applications.md by hand).
...
export async function POST(req: Request) {
  ...
  if ((kind === "evaluate" || kind === "pdf") && !fs.existsSync(path.join(userDataRoot(session), "cv.md"))) {
    return new Response(
  ...
  // In multi-tenant mode, prefix user-layer paths so the CLI writes to the right
  // user directory. In single-user mode, prefix is "" (no change).
  const userPrefix = session ? `users/${session}/` : "";
  const prompt = buildPrompt(kind, input, readMemory(session), today, userPrefix);
  ...
  const reportsDir = path.join(userDataRoot(session), "reports");
  const countReports = () => {
    try {
