...
import { careerOpsRoot, userDataRoot, readMemory, doctorState } from "@/lib/career-ops";
...
export async function POST(req: Request) {
  ...
  // (doctorState), so it never re-asks for a file that already exists —
  ...
  const { hasCv, onboardingNeeded, missing } = doctorState(session);
  ...
  // In multi-tenant mode, load user-specific env variables (custom API keys, etc.)
