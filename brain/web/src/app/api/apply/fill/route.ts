...
export async function POST(req: Request) {
  ...
  const appSession = getSession(sessionId);
  const cvPath = resolveTailoredCv(company, session) ?? resolveTailoredCv(companyFromTitle(appSession?.title), session) ?? undefined;
