...
export async function POST(req: Request) {
  ...
  const dataRoot = userDataRoot(session);
  const logPath = path.join(dataRoot, ".career-ops-web", "apply-prefill.log");
  try {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      const fieldsList = s.fields
        ...
        .join("\n");
      const mem = readMemory(session).trim();
...
Output ONLY a compact JSON object mapping each field id → {"value": "...", "needs_confirmation": boolean}. No prose, no markdown, no code fence.`;
      ...
      // Load user-specific .env variables for custom keys
      const userEnvFile = path.join(dataRoot, ".env");
      const userEnv: Record<string, string> = {};
      if (fs.existsSync(userEnvFile)) {
        try {
          const content = fs.readFileSync(userEnvFile, "utf8");
      ...
      const result = await new Promise<{ buf: string; code: number | null; signal: NodeJS.Signals | null }>((resolve) => {
        ...
        const child = spawn(binPath, args, { cwd: dataRoot, env: { ...process.env, ...userEnv }, stdio: ["ignore", "pipe", "pipe"] });
        let buf = "";
        let firstByteAt = 0;
