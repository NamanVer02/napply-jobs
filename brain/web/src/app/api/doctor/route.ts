...
import { careerOpsRoot, userDataRoot, rootScript } from "@/lib/career-ops";
...
// Multi-tenant aware: targets the user's isolated directory under users/<name>/.
export async function GET() {
  const session = await requireSession();
  if (session instanceof Response) return session;

  const root = careerOpsRoot();
  const target = userDataRoot(session);
  ...
  const stdout = await new Promise<string>((resolve) => {
    execFile("node", [doctor, "--json", "--target", target], { cwd: root, timeout: 10_000 }, (_err, out) => resolve(out || ""));
  });
