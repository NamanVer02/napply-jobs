...
export function MobileNav() {
  ...
  const { jobs } = useJobs();
  const { username, logout, multiTenant } = useAuth();
    ...
    <>
      ...
      >
        ...
        <div className="co-msafe mt-auto space-y-3 border-t border-border px-4 pt-4">
          {multiTenant && username && (
            <div className="flex items-center justify-between rounded-lg border border-border/50 bg-surface/20 px-3 py-2 text-xs">
              ...
              <button
                onClick={logout}
                title="Sign out"
                className="rounded p-1 text-faint hover:bg-surface-hover hover:text-foreground transition-colors"
              >
