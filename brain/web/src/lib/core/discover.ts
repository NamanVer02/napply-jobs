...
import { userDataRoot, readInbox, readApplications } from "@/lib/career-ops";
 ...
 */
export function assembleDedupContext(username?: string): { urls: Set<string>; lines: string[] } {
  ...
  try {
    const tsv = fs.readFileSync(path.join(userDataRoot(username), "data", "scan-history.tsv"), "utf8");
