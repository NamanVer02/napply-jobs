...
import { careerOpsRoot, userDataRoot } from "@/lib/career-ops";
...
function loadYaml(rel: string, username?: string): Record<string, unknown> | null {
  try {
    const doc = yaml.load(fs.readFileSync(path.join(userDataRoot(username), rel), "utf8"));
 ...
 */
export function seedExploreFilters(username?: string): { filters: ExploreFilters; seededFrom: string[] } {
  const filters: ExploreFilters = { ...DEFAULT_FILTERS, ats: [...DEFAULT_FILTERS.ats] };
  const seededFrom: string[] = [];

  const portals = loadYaml("portals.yml", username);
  if (portals) {
  ...
  if (filters.positive.length === 0) {
    const profile = loadYaml("config/profile.yml", username);
