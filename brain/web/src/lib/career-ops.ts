...
/**
 * Resolve the career-ops "home" — the directory holding the system scripts,
 * modes, templates. In production the web/ app lives inside the career-ops
 * checkout, so the home is its parent (..). Dev overrides via CAREER_OPS_ROOT.
 */
...
/**
 * Resolve the per-user data root. In multi-tenant mode, this returns
 * `users/<username>/` within the career-ops checkout. In single-user mode
 * (username is undefined), falls back to careerOpsRoot() — zero behavior change.
 */
export function userDataRoot(username?: string): string {
  if (!username) return careerOpsRoot();
  return path.join(careerOpsRoot(), "users", username);
}
...
function read(rel: string, username?: string): string | null {
  try {
    return fs.readFileSync(path.join(userDataRoot(username), rel), "utf8");
  } catch {
    return null;
  }
}
...
export function readInbox(username?: string): InboxJob[] {
  const md = read("data/pipeline.md", username);
  if (!md) return [];
  const jobs: InboxJob[] = [];
 ...
 */
export function readScanDates(username?: string): Map<string, string> {
  const tsv = read("data/scan-history.tsv", username);
 ...
 */
export function readApplications(username?: string): Application[] {
  const md = read("data/applications.md", username);
  if (!md) return [];
  return parseApplications(md, userDataRoot(username));
}
 ...
 */
export function doctorState(username?: string): {
  ...
  const root = userDataRoot(username);
  ...
  const hasData = readApplications(username).length > 0 || readInbox(username).some((j) => !j.done);
...
export function pipelineSummary(username?: string): PipelineSummary {
  const root = userDataRoot(username);
  const scanDates = readScanDates(username);
  return {
    root,
    ...
    inbox: readInbox(username).map((j) => ({ ...j, postedAt: scanDates.get(j.url) })),
    applications: readApplications(username),
  };
}
...
export function findReportFile(n: string, username?: string): string | null {
  const target = parseInt(n, 10);
  if (Number.isNaN(target)) return null;
  let files: string[];
  try {
    files = fs.readdirSync(path.join(userDataRoot(username), "reports"));
  } catch {
    return null;
  }
  const match = files.find((f) => f.endsWith(".md") && parseInt(f, 10) === target);
  return match ? path.join(userDataRoot(username), "reports", match) : null;
}

export function readReport(n: string, username?: string): ReportData | null {
  const file = findReportFile(n, username);
  if (!file) return null;
  try {
...
export function findApplication(n: string, username?: string): Application | null {
  return readApplications(username).find((a) => a.n === n) ?? null;
}
...
export function profilePath(username?: string): string {
  return path.join(userDataRoot(username), "modes", "_profile.md");
}
...
export function readMemory(username?: string): string {
  try {
    const md = fs.readFileSync(profilePath(username), "utf8");
  ...
  try {
    return fs.readFileSync(path.join(userDataRoot(username), ".career-ops-web", "memory.md"), "utf8").trim();
  } catch {
    return "";
  }
}
...
export function rememberFact(fact: string, username?: string): "ok" | "deduped" | "error" {
